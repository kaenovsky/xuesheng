...
The application renders **“Hello, <span style={{ color: 'red' }}>John Doe</span>”** as the heading, the name will be in red because it matches the `type: 'admin'{:js}` variant.
...
4. Since Stitches already knows which CSS rules are used (as step 2.), it can then return it via `getCssText(){:js}`. And the rendered HTML string will look like:
...
5. Then, Next.js replaces `__PLACEHOLDER__{:js}` above with the rendered app content from step 1.: `<h1>Hello, <span className="stitches-xyz stitches-xyz-type-admin">John Doe</span></h1>{:html}`. Now everything is finished and all styles are server-side rendered.
